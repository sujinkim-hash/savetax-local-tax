...
export default function Home() {
  ...
  return <main className="appShell">
    <aside className="sideRail" aria-label="주요 메뉴"><div className="railBrand"><span>LOCAL TAX</span><strong>지방소득세<br />담당자 조회</strong></div><nav><a className="active" href="#directory">연락처 조회</a><Link href="/review">검토 현황</Link><a href="#admin-tools">관리자 도구</a></nav><p>종합소득세 관련<br />담당 주무관 연락처</p></aside>
    ...
    {isAdmin && <section className="review" id="admin-tools"><div><p className="eyebrow">ADMINISTRATION</p><h2>관리자 검토 및 자동화</h2><p>이 영역은 관리자 키 인증 후에만 보입니다. 승인한 변경만 공개 연락처에 반영됩니다.</p>{reviews.length > 0 && <details className="reviewList"><summary>검토 대상 {reviews.length}건 보기</summary><ul>{reviews.map((review) => <li key={review.id}><b>{review.sido} {review.local}</b> · {review.field}: {review.previous_value} → {review.source_url ? <a href={review.source_url} target="_blank" rel="noreferrer">공식 페이지 열기</a> : review.proposed_value}<button onClick={() => approveReview(review.id)}>승인</button></li>)}</ul></details>}</div><div className="reviewActions"><button onClick={initializeDatabase}>연락처 DB 시작하기</button><button onClick={importMoisSources}>행안부 주소 일괄 수집</button><button className="secondary" onClick={() => { setEditingSourceId(null); setSourceUrlDraft(""); setSourceSidoDraft(""); setSourceLocalDraft(""); setSourceDialogOpen(true); }}>공식 주소 직접 등록</button></div>{sources.length > 0 && <details className="sourceList sourceListAccordion"><summary><div><p className="eyebrow">OFFICIAL ADDRESS STATUS</p><h3>공식 주소 등록 현황</h3></div><span>등록 {sources.length}개</span><i aria-hidden="true" /></summary><div className="sourceListBody"><p>시·도를 선택하면 해당 지자체의 공식 직원검색·조직도 주소를 확인할 수 있습니다.</p><div className="sourceListRegions">{sourcesBySido.map((group) => <details className="sourceListRegion" key={group.sido}><summary><b>{group.sido}</b><span>{group.items.length}개 지자체</span></summary><ul>{group.items.map((source) => <li key={source.id}><div className="sourceMeta"><b>{source.local}</b><span>등록일 {String(source.created_at).slice(0, 10)}</span></div><div className="sourceActions"><a href={source.source_url} target="_blank" rel="noreferrer">공식 페이지 열기</a><button type="button" onClick={() => editSource(source)}>수정</button><button type="button" className="sourceDelete" onClick={() => void deleteSource(source)}>삭제</button></div></li>)}</ul></details>)}</div></div></details>}</section>}
