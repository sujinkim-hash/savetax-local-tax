...
export default function Home() {
  ...
  async function addSource() {
    ...
    if (!key) return;
    const sourceUrl = window.prompt("1/3. 공식 직원검색·조직도 웹페이지 주소를 붙여 넣으세요.");
    if (!sourceUrl) return;
    const sido = window.prompt("2/3. 시·도를 입력하세요. 예: 경기도 또는 서울특별시");
    if (!sido) return;
    const local = window.prompt("3/3. 시·군·구를 입력하세요. 예: 성남시 또는 은평구");
    if (!local) return;
    setAdminKey(key);
  ...
  return <main>
      <header><div><p className="eyebrow">LOCAL INCOME TAX DIRECTORY</p><h1>전국 지방소득세 담당자 연락처</h1><p className="lead">종합소득세와 관련된 담당 주무관 기준으로 전국 시·군·구 연락처를 한 곳에서 확인합니다.</p></div><div className="headerActions"><Link className="reviewLink" href="/review">검토 현황</Link><button className="admin" onClick={isAdmin ? () => { setIsAdmin(false); setReviews([]); setNotice("관리자 모드를 종료했습니다."); } : authenticateAdmin}>{isAdmin ? "관리자 모드 종료" : "관리자"} {!isAdmin && <span>🔒</span>}</button></div></header>
