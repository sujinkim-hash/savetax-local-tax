...
async function syncSourceReviews(sql: NonNullable<Awaited<ReturnType<typeof ensureSchema>>>) {
  await sql`INSERT INTO review_candidates (contact_id, sido, local_name, field, previous_value, proposed_value, reason, source_url, status) SELECT NULL, s.sido, s.local_name, '공식 홈페이지', '기존 미등록', s.source_url, '등록된 공식 직원검색·조직도 주소입니다. 주소와 지자체를 확인한 뒤 반영하세요.', s.source_url, 'pending' FROM source_pages s WHERE NOT EXISTS (SELECT 1 FROM review_candidates r WHERE r.sido = s.sido AND r.local_name = s.local_name AND r.field = '공식 홈페이지' AND r.proposed_value = s.source_url)`;
}
...
export async function POST(request: Request) {
  ...
  await sql`DELETE FROM review_candidates WHERE sido = ${body.sido} AND local_name = ${body.local} AND field = '공식 홈페이지' AND status = 'pending'`;
  ...
  await sql`INSERT INTO review_candidates (contact_id, sido, local_name, field, previous_value, proposed_value, reason, source_url, status) VALUES (NULL, ${body.sido}, ${body.local}, '공식 홈페이지', '기존 미등록', ${body.sourceUrl}, '관리자가 등록한 공식 직원검색·조직도 주소입니다. 주소와 지자체를 확인한 뒤 반영하세요.', ${body.sourceUrl}, 'pending')`;
...
export async function DELETE(request: Request) {
  ...
  if (source) await sql`DELETE FROM review_candidates WHERE sido = ${source.sido} AND local_name = ${source.local} AND field = '공식 홈페이지' AND proposed_value = ${source.source_url} AND status = 'pending'`;
