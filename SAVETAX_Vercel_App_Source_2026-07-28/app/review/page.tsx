...
export default function ReviewPage() {
  ...
  return <main>
    ...
    <section className="sourceReviewPanel"><div className="panelHead"><div><p className="eyebrow">OFFICIAL SOURCES</p><h2>공식 주소 등록 현황</h2></div><p>관리자가 직접 등록한 지자체 직원검색·조직도 주소입니다.</p></div>{sourcesLoaded && sources.length > 0 ? <ul className="sourceReviewList">{sources.map((source) => <li key={source.id}><div><b>{source.sido} {source.local}</b><span>등록일 {source.created_at.slice(0, 10)}</span></div><a href={source.source_url} target="_blank" rel="noreferrer">공식 페이지 열기</a></li>)}</ul> : <p className="sourceReviewEmpty">{sourcesLoaded ? "아직 등록된 공식 주소가 없습니다." : "등록된 공식 주소를 불러오는 중입니다."}</p>}</section>
  </main>;
}
