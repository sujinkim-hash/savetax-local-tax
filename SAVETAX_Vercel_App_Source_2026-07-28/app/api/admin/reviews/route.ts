...
export async function GET(request: Request) {
  ...
  const reviews = await sql`SELECT id, sido, local_name AS local, field, previous_value, proposed_value, reason, created_at FROM review_candidates WHERE status = 'pending' ORDER BY created_at DESC`;
