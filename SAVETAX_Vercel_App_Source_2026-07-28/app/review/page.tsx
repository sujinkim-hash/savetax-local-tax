...
if (savedKey) void verifyAdminKey(savedKey, true);
  ...
  const visibleReviews = (reviewStatusFilter === "registered" ? registeredReviews : unregisteredReviews)
    .filter((review) => [review.sido, review.local].join(" ").toLowerCase().includes(reviewSearch.trim().toLowerCase()));
  const reviewGroups = Array.from(new Set(visibleReviews.map((review) => review.sido)))
    .sort((left, right) => left.localeCompare(right, "ko"))
    .map((sido) => ({ sido, rows: visibleReviews.filter((review) => review.sido === sido) }));
