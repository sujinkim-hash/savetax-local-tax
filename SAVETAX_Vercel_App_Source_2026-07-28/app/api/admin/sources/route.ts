...
export async function POST(request: Request) {
  ...
  const sql = await ensureSchema();
  if (!sql) return Response.json({ error: "DATABASE_URL 연결을 확인하세요." }, { status: 503 });
  await sql`INSERT INTO source_pages (sido, local_name, source_url) VALUES (${body.sido}, ${body.local}, ${body.sourceUrl}) ON CONFLICT (sido, local_name, source_url) DO NOTHING`;
