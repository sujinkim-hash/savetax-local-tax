...
export async function PATCH(request: Request) {
  const body = (await request.json()) as { action?: "deleteSourceNote" | "updateSourceNote"; id?: number; sido?: string; local?: string; navigationNote?: string };
  const sql = await ensureSchema();
  if (!sql) return Response.json({ error: "DATABASE_URL 연결을 확인하세요." }, { status: 503 });
  ...
  if (body.action === "updateSourceNote") {
    if (!body.id || !body.navigationNote?.trim()) return Response.json({ error: "수정할 메모 내용을 입력하세요." }, { status: 400 });
    await sql`UPDATE source_pages SET navigation_note = ${body.navigationNote.trim()} WHERE id = ${body.id}`;
    return Response.json({ ok: true, sourceNoteUpdated: true });
  }
