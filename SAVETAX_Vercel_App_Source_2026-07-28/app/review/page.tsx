...
if (savedKey) void verifyAdminKey(savedKey, true);
  ...
  async function saveSource(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/admin/sources", {
      ...
      body: JSON.stringify({ id: editingSource.id || undefined, sido: sourceSidoDraft.trim(), local: sourceLocalDraft.trim(), sourceUrls: [{ sourceUrl: sourceUrlDraft.trim(), navigationNote: sourceNoteDraft.trim() }, ...(editingSource.id ? [] : sourceExtraUrlDraft.trim() ? [{ sourceUrl: sourceExtraUrlDraft.trim(), navigationNote: sourceExtraNoteDraft.trim() }] : [])] }),
    });
    const data = (await response.json()) as { error?: string };
    if (!response.ok) {
  ...
  return <main className="reviewShell">
    ...
    {editingSource && <div className="dialogBackdrop" role="presentation"><form className="dialog" onSubmit={saveSource}><h2>{editingSource.id ? "공식 주소 수정" : "공식 주소 등록"}</h2><p>{editingSource.id ? "지역 이름과 직원검색·조직도 주소를 함께 수정할 수 있습니다." : "해당 지자체의 공식 직원검색·조직도 주소를 입력해 등록하세요."}</p><label htmlFor="source-edit-sido">시·도</label><input id="source-edit-sido" required readOnly={editingSource.id === 0} value={sourceSidoDraft} onChange={(event) => setSourceSidoDraft(event.target.value)} placeholder="예: 경기도" /><label htmlFor="source-edit-local">시·군·구</label><input id="source-edit-local" required readOnly={editingSource.id === 0} value={sourceLocalDraft} onChange={(event) => setSourceLocalDraft(event.target.value)} placeholder="예: 성남시" /><label htmlFor="source-edit-url">공식 직원검색·조직도 주소</label><input id="source-edit-url" type="url" autoFocus required value={sourceUrlDraft} onChange={(event) => setSourceUrlDraft(event.target.value)} placeholder="https://" /><label htmlFor="source-edit-note">확인 경로 메모 <small>(선택)</small></label><textarea id="source-edit-note" value={sourceNoteDraft} onChange={(event) => setSourceNoteDraft(event.target.value)} placeholder="예: 세무2과 선택 후 지방소득세(종합소득) 담당자 확인" />{!editingSource.id && <><label htmlFor="source-edit-extra-url">추가 공식 주소 <small>(선택)</small></label><input id="source-edit-extra-url" type="url" value={sourceExtraUrlDraft} onChange={(event) => setSourceExtraUrlDraft(event.target.value)} placeholder="두 번째 직원검색·조직도 주소 (https://)" /><label htmlFor="source-edit-extra-note">추가 주소 확인 경로 메모 <small>(선택)</small></label><textarea id="source-edit-extra-note" value={sourceExtraNoteDraft} onChange={(event) => setSourceExtraNoteDraft(event.target.value)} placeholder="두 번째 주소의 확인 경로를 적어주세요" /></>}<div className="dialogActions"><button type="button" className="dialogCancel" onClick={() => { setEditingSource(null); setSourceUrlDraft(""); setSourceSidoDraft(""); setSourceLocalDraft(""); setSourceNoteDraft(""); setSourceExtraUrlDraft(""); setSourceExtraNoteDraft(""); }}>취소</button><button type="submit">{editingSource.id ? "저장" : "등록"}</button></div></form></div>}
      </div>
    </div>
  </main>;
}
