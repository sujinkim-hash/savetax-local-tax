...
if (savedKey) void verifyAdminKey(savedKey, true);
  ...
  const officeRows = useMemo(() => {
    ...
    const offices = new Map<string, { sido: string; local: string; sources: Source[] }>();
    contacts.forEach((contact) => {
      const key = contact.sido + "::" + contact.local;
      if (!offices.has(key)) offices.set(key, { sido: contact.sido, local: contact.local, sources: sourceMap.get(key) ?? [] });
    });
  ...
  const registeredOffices = officeRows.filter((office) => office.sources.length > 0);
  const unregisteredOffices = officeRows.filter((office) => office.sources.length === 0);
  ...
  return <main className="reviewShell">
    ...
    <details className="reviewPanel sectionAccordion" onToggle={(event) => { if (event.currentTarget.open) window.setTimeout(() => event.currentTarget.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }}>
      <summary className="panelHead"><div><p className="eyebrow">CHANGE REVIEW</p><h2>수정 내용 검토</h2></div><p>공식 주소와 담당 연락처의 변경 사항을 검토합니다.</p><i aria-hidden="true" /></summary>
      <div className="changeReviewBody">
        {isAdmin && <button type="button" className="reviewAddContactButton" onClick={openContactAdd}>+ 연락처 추가</button>}
        {!isAdmin ? <button type="button" className="reviewAdminButton" onClick={() => setAdminDialogOpen(true)}>관리자 인증 후 변경 검토</button> : (registeredOffices.length + unregisteredOffices.length) > 0 ? <>
          <div className="reviewStatusTabs" role="group" aria-label="검토 상태 선택"><button type="button" className={reviewStatusFilter === "registered" ? "active" : ""} onClick={() => setReviewStatusFilter("registered")}>등록 <span>{registeredOffices.length}</span></button><button type="button" className={reviewStatusFilter === "unregistered" ? "active" : ""} onClick={() => setReviewStatusFilter("unregistered")}>미등록 <span>{unregisteredOffices.length}</span></button></div>
          <input type="text" className="reviewSearchInput" aria-label="지역 검색" placeholder="시·도 또는 시·군·구 검색" value={reviewSearch} onChange={(event) => setReviewSearch(event.target.value)} />
          {visibleOffices.length > 0 ? <div className="reviewRegionGroups">{officeGroups.map((group) => <details className="reviewRegion" key={group.sido}><summary><span className="reviewRegionName">{group.sido}</span><span className="reviewRegionCount">{group.rows.length}건</span></summary><ul className="changeReviewList">{group.rows.map((office) => {
            const homepageCandidate = homepageCandidateByOffice.get(office.sido + "::" + office.local) ?? findOfficialHomepage(homepageCandidates, office);
            return <li key={office.sido + office.local} className="reviewCandidateRow"><b>{office.local}</b><div className="changeReviewActions">{reviewStatusFilter === "registered" ? <>
              {office.sources.map((source, index) => <a key={source.id} href={source.source_url} target="_blank" rel="noreferrer">홈페이지{office.sources.length > 1 ? " " + (index + 1) : ""}</a>)}
              <button type="button" className="reviewContactAction" onClick={() => openOfficeContacts(office)}>연락처 확인·수정</button>
              {office.sources.map((source, index) => <button key={source.id} type="button" onClick={() => openSourceEditor(source)}>주소 수정{office.sources.length > 1 ? " " + (index + 1) : ""}</button>)}
            </> : <>
              {homepageCandidate && <a href={homepageCandidate.source_url} target="_blank" rel="noreferrer">공식 페이지 확인</a>}
              <button type="button" className="reviewContactAction" onClick={() => openOfficeContacts(office)}>연락처 확인·수정</button>
              <button type="button" onClick={() => openSourceEditor({ id: 0, sido: office.sido, local: office.local, source_url: "" })}>주소 등록</button>
            </>}</div></li>;
          })}</ul></details>)}</div> : <p className="sourceReviewEmpty">{reviewStatusFilter === "registered" ? "등록 상태의 검토 항목이 없습니다." : "미등록 상태의 검토 항목이 없습니다."}</p>}
        </> : <p className="sourceReviewEmpty">현재 반영 대기 중인 수정 내용이 없습니다.</p>}
      </div>
    </details>
