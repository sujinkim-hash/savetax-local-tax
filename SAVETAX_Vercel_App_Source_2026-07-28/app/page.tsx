...
export default function Home() {
  ...
  const officeNoteByOffice = useMemo(() => new Map(officeNotes.map((note) => [note.sido + "::" + note.local, note.navigation_note])), [officeNotes]);
  ...
  async function loadSources(key: string) {
    ...
    if (!response.ok) return;
    const data = (await response.json()) as { sources?: Source[]; officeNotes?: OfficeNote[] };
    setSources(data.sources ?? []);
    setOfficeNotes(data.officeNotes ?? []);
  }
  ...
  async function addSource() {
    ...
    if (!adminKey || !sido || !local || (!sourceUrl && !sourceNoteDraft.trim())) return;
    const response = await fetch("/api/admin/sources", { method: "POST", headers: { "content-type": "application/json", "x-admin-key": adminKey }, body: JSON.stringify({ id: editingSourceId ?? undefined, sido, local, sourceUrl, navigationNote: sourceNoteDraft.trim(), noteOnly: !sourceUrl }) });
  ...
  async function saveContactPhone() {
    ...
    const response = await fetch("/api/contacts", { method: "PATCH", headers: { "content-type": "application/json", "x-admin-key": adminKey }, body: JSON.stringify({ id: editingContact.id, local: contactLocalDraft.trim(), scope: contactScopeDraft.trim(), phone: contactPhoneDraft.trim() }) });
    const data = (await response.json()) as { error?: string };
    if (!response.ok) { setNotice(data.error ?? "직통번호 수정에 실패했습니다."); return; }
    const newPhone = contactPhoneDraft.trim();
    const newLocal = contactLocalDraft.trim();
    const newScope = contactScopeDraft.trim();
    setContacts((current) => current.map((item) => item.id === editingContact.id ? { ...item, local: newLocal, scope: newScope, phone: newPhone, checked: new Date().toISOString().slice(0, 10), status: "확인" } : item));
    ...
    <section className="panel" id="directory">
      ...
      <div className="table">
        ...
        {pagedRows.map((item, index) => {
          ...
          const officeNote = officeNoteByOffice.get(item.sido + "::" + item.local);
